...
namespace RVO {
	...
	void Agent::computeNewVelocity()
	{
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			// Relative pos/v are defined as:
			// const Vector2 relativePosition = other->position_ - position_;
			// const Vector2 relativeVelocity = velocity_ - other->velocity_;
			float vp_dotProduct = relativePosition.x() * relativeVelocity.x() + relativePosition.y() * relativeVelocity.y();
			float norm_relativePosition = std::sqrt(relativePosition.x() * relativePosition.x() + relativePosition.y() * relativePosition.y());
			float norm_relativeVelocity = std::sqrt(relativeVelocity.x() * relativeVelocity.x() + relativeVelocity.y() * relativeVelocity.y());
			float vp_angle_cos = vp_dotProduct / (norm_relativePosition * norm_relativeVelocity);
			
			float param = 0.5f;
			if (vp_angle_cos >= 0) {
				param = 0.7 + 0.3 * vp_angle_cos;
			}
			
			line.point = velocity_ + param * u;
